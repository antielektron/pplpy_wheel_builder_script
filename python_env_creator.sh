...
perform_and_exit wget -O ~/src/python.tar.xz https://www.python.org/ftp/python/`echo $PYTHON_VERSION | cut -db -f1`/Python-$PYTHON_VERSION.tar.xz
...
then
    ...
    then
        ...
        message "ERROR: no virtualenv found"
        exit -1
    fi
fi
...
perform_and_exit $VIRTUALENV_BIN --python=$PYTHON_DIR/bin/python$PYTHON_VERSION_MAJOR_MINOR $HOME/envs/python$PYTHON_VERSION
